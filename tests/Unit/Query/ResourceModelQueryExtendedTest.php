...
final class ResourceModelQueryExtendedTest extends TestCase
{
    ...
    #[Test]
    public function where_raw_ignores_question_marks_inside_sql_comments(): void
    {
        $query = $this->query();

        $query
            ->forTenant('tenant-1')
            ->whereRaw("JSON_EXTRACT(`name`, ?) = ? /* comment ? */ -- tail ?\n", ['$.type', 'pro']);

        $sql = $query->toSql();
        $this->assertStringContainsString('JSON_EXTRACT(`name`, :raw0) = :raw1', $sql);
        $this->assertSame('$.type', $query->toParams()['raw0']);
        $this->assertSame('pro', $query->toParams()['raw1']);
    }
    ...
    #[Test]
    public function to_debug_sql_ignores_parameter_like_text_inside_sql_literals(): void
    {
        $query = $this->query();
        $query
            ->forTenant('tenant-1')
            ->whereRaw("JSON_EXTRACT(`name`, '$.:tenant_scope') = ?", ['pro']);

        $debug = $query->toDebugSql();
        $this->assertStringContainsString("JSON_EXTRACT(`name`, '$.:tenant_scope') = 'pro'", $debug);
        $this->assertStringContainsString("'tenant-1'", $debug);
    }
